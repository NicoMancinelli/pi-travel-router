...
# g_ether (CDC-ECM) is natively supported out-of-the-box by macOS (AppleUSBNetworking)
# and Linux, and provides RNDIS fallback on Windows.
...
if [ ! -f "$CMDLINE_TXT" ]; then
    CMDLINE_TXT="${ROOTFS_DIR}/boot/config.txt"
    [ ! -f "$CMDLINE_TXT" ] || CMDLINE_TXT="${ROOTFS_DIR}/boot/cmdline.txt"
fi
