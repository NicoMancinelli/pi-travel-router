...
def api_system_smart():
    ...
    try:
        ...
        for dev_path in sorted(sys_block.iterdir()):
            ...
            if rc not in (0, 4):
                # rc=4 means some SMART data returned but device has issues
                # rc=1 means smartctl couldn't open device (common for SD cards / eMMC)
            ...
            for line in out.splitlines():
                ...
                if len(parts) >= 10:
                    ...
                    if attr_name in ("Temperature_Celsius", "Airflow_Temperature_Cel"):
                        # RAW_VALUE may have extra info like "35 (Min/Max 25/45)"
                        try:
