...
@app.route("/api/vpn/wireguard/peer/<path:pubkey>", methods=["DELETE"])
@require_auth_always
def api_wg_delete_peer(pubkey):
    """Remove a WireGuard peer from wg0.conf by public key."""
    # Validate key format before doing anything
    if not re.match(r'^[A-Za-z0-9+/]{43}=$', pubkey):
        return jsonify({"error": "Invalid public_key format"}), 400

    try:
        content = Path(WG_CONF).read_text()
    except OSError as e:
        return jsonify({"error": f"Cannot read {WG_CONF}: {e}"}), 503

    if pubkey not in content:
        return jsonify({"error": "Peer not found"}), 404

    # Remove the [Peer] block that contains this public key.
    # A block starts at [Peer] and ends at the next [section] or EOF.
    new_lines = []
    skip = False
    for line in content.splitlines(keepends=True):
        if line.strip() == "[Peer]":
            # Look ahead: peek at upcoming lines for this pubkey.
            # We use a two-pass approach: first collect the block.
            skip = False
        if f"PublicKey = {pubkey}" in line:
            # Remove the preceding [Peer] line already appended
    ...
    try:
        Path(WG_CONF).write_text("".join(new_lines))
    except OSError as e:
        return jsonify({"error": f"Write failed: {e}"}), 503

    # Reload live config if interface is up
    try:
        result = subprocess.run(["ip", "link", "show", "wg0"], capture_output=True, timeout=3)
        if result.returncode == 0:
            subprocess.run(["wg", "syncconf", "wg0", WG_CONF], capture_output=True, timeout=5)
    except Exception:
        pass

    return jsonify({"ok": True, "deleted": pubkey})


# ── Privacy profiles ──────────────────────────────────────────────────────────

_PRIVACY_PROFILES = ("standard", "private", "paranoid")
_PRIVACY_STATE_FILE = "/var/lib/travel-router/privacy-profile"


def _current_privacy_profile():
    try:
        p = Path(_PRIVACY_STATE_FILE).read_text().strip()
        if p in _PRIVACY_PROFILES:
            return p
    except OSError:
        pass
    return "standard"
...
def api_privacy_get():
    return jsonify({
        "active": _current_privacy_profile(),
        "profiles": list(_PRIVACY_PROFILES),
    })
...
def api_privacy_set():
    ...
    if not isinstance(data, dict):
        return jsonify({"error": "Expected JSON object"}), 400
    profile = data.get("profile", "")
    if profile not in _PRIVACY_PROFILES:
        return jsonify({"error": f"Unknown profile: {profile!r}. Valid: {list(_PRIVACY_PROFILES)}"}), 400
    try:
        Path(_PRIVACY_STATE_FILE).parent.mkdir(parents=True, exist_ok=True)
        Path(_PRIVACY_STATE_FILE).write_text(profile + "\n")
    except OSError as e:
        return jsonify({"error": f"Write failed: {e}"}), 503
