...
### Added
- `GET /api/system/irq-stats` — top IRQ consumers from /proc/interrupts with chip and action
- `GET /api/system/disk-partitions` — mounted partition info via /proc/mounts + statvfs (new card for existing endpoint)
- `GET /api/system/process-count` — process counts by state (R/S/D/T/Z) with load average

## [3.27.0] — 2026-05-26

### Added
- `GET /api/system/numa-info` — NUMA topology from /sys/devices/system/node with UMA fallback
- `GET /api/network/socket-stats` — socket summary via ss -s with /proc/net/sockstat fallback
- `GET /api/system/kernel-version` — kernel release/arch/build + OS pretty name from /etc/os-release
...
### Added
- `GET /api/system/cpu-frequency` — per-core CPU MHz from sysfs with /proc/cpuinfo fallback
- `GET /api/network/route-table` — kernel routing table via ip -j route with default route summary
- `GET /api/system/shared-memory` — IPC shm segments via ipcs -m plus kernel Shmem from /proc/meminfo

## [3.24.0] — 2026-05-26

### Added
- `GET /api/system/dmesg-errors` — dmesg kernel error/warning extraction with healthy flag
- `GET /api/network/interface-stats` — per-interface TX/RX bytes/packets from /proc/net/dev sorted by traffic
- `GET /api/network/resolved-dns` — active DNS server list via resolvectl with /etc/resolv.conf fallback
